...
parser.add_argument('--config_file', 		type=str, default="/data/lmx/CMT/heatmaps/configs/config_template.yaml")
parser.add_argument('--data_root_dir',      type=str, default='/data/lmx/Dataset/TCGA-DATASET/FEATURES_DIRECTORY/tcga_blca',
                                            help='data directory to WSI features (extracted via CLAM)')
parser.add_argument('--split_dir',          type=str, choices = ['tcga_blca', 'tcga_luad', 'tcga_brca'], default='tcga_blca', help='cancer type')
parser.add_argument('--task',               type=str, choices=['task_1_tumor_vs_normal',  'task_2_tumor_subtyping', 'task_3_survival_prediction'],
                                            default='task_3_survival_prediction')
args = parser.parse_args()


def infer_single_slide(model, features, omics, label, reverse_label_dict, k=1):
	features = features.to(device)
	omics = omics.type(torch.FloatTensor).to(device)
	with torch.no_grad():
		if isinstance(model, (CLAM_SB, CLAM_MB)):
			model_results_dict = model(features)
			logits, Y_prob, Y_hat, A, _ = model(features) # (1,2), (1,2), (1,1), (2,7336)
			print("Y_hat: ", Y_hat)
			Y_hat = Y_hat.item() # 0
			if isinstance(model, (CLAM_MB,)):
				A = A[Y_hat] # (7336,)

			A = A.view(-1, 1).cpu().numpy() # (7336,1)

		# TODO: code of MGCT creating heatmap,
		elif isinstance(model, MGCT):
			model_result_dict = model(features, omics)
			logits, Y_prob, _, _, Y_hat, A = model(features, omics) # (1,4), (1,4), (1,1), (7336,1)
			Y_hat = Y_hat.item() # 3
			# A = A[Y_hat] #
			# print("A[Y_hat].shape: ", A.shape)
			A = A.view(-1, 1).cpu().numpy()
		else:
			raise NotImplementedError
		# 'subtype_0', 'subtype_0', ['0.6145', '0.3855']
		print('Y_hat: {}, Y: {}, Y_prob: {}'.format(reverse_label_dict[Y_hat], label, ["{:.4f}".format(p) for p in Y_prob.cpu().flatten()]))	
		
		probs, ids = torch.topk(Y_prob, k) # (1,2), (1,2)
		probs = probs[-1].cpu().numpy() # (2,)
		ids = ids[-1].cpu().numpy() # [0,1]
		preds_str = np.array([reverse_label_dict[idx] for idx in ids]) # ['subtype_0','subtype_1']
...
if __name__ == '__main__':
	...
	for i in range(len(process_stack)):
		...
		train_dataset, val_dataset = dataset.return_splits(from_id=False, csv_path='/data/lmx/CMT/splits/5foldcv/tcga_blca/splits_0.csv')
		...
		for batch_idx, (data_WSI, data_omic, label, event_time, censor) in enumerate(omic_loader):
			features = torch.load(features_path)  # (7336,1024)
			process_stack.loc[i, 'bag_size'] = len(features)  # 7336
			wsi_object.saveSegmentation(mask_file)
			data_omic = data_omic.type(torch.FloatTensor).to(device)
			Y_hats, Y_hats_str, Y_probs, A = infer_single_slide(model, features, data_omic, label, reverse_label_dict,
																exp_args.n_classes)
			del features
		...
		# process_stack.loc[i, 'bag_size'] = len(features) # 7336
		# wsi_object.saveSegmentation(mask_file)
		# # [0,1], ['subtype_0','subtype_1'], (2,), (7336,1)
