...
def MLP_Block(in_dim, out_dim, dropout=0.1):
    return nn.Sequential(
            nn.Linear(in_dim, out_dim//2), nn.ReLU(), nn.Dropout(p=dropout),
            nn.Linear(out_dim//2, out_dim//2), nn.ReLU(), nn.Dropout(p=dropout))
